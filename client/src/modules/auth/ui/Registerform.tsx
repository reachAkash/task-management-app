...
import { loginRoute, otpRoute } from "@/axios/apiRoutes";
...
export function RegisterForm({
  ...
  const RegisterUser = async () => {
    try {
      setLoading(true);
      const { data }: { data: RegisterForm } = await axiosInstance.post(
        loginRoute,
        { email, password }
      );
      console.log(data);
  ...
  const verifyOtp = async () => {
    try {
      setLoading(true);
      const { data }: { data: OtpResponse } = await axiosInstance.post(
        otpRoute,
        {
          email,
          otp,
